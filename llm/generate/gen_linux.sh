...
COMMON_CMAKE_DEFS="-DCMAKE_POSITION_INDEPENDENT_CODE=on -DLLAMA_NATIVE=off -DLLAMA_AVX=on -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=off -DLLAMA_FMA=off -DLLAMA_F16C=off"
...
if [ -z "${OLLAMA_SKIP_STATIC_GENERATE}" -o "${OLLAMA_CPU_TARGET}" = "static" ]; then
    ...
    CMAKE_DEFS="-DBUILD_SHARED_LIBS=off -DLLAMA_NATIVE=off -DLLAMA_AVX=off -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=off -DLLAMA_FMA=off -DLLAMA_F16C=off ${CMAKE_DEFS}"
...
if [ -z "${OLLAMA_SKIP_CPU_GENERATE}" ]; then
    ...
    if [ -n "${OLLAMA_CUSTOM_CPU_DEFS}" ]; then
        ...
        if [ -z "${OLLAMA_CPU_TARGET}" -o "${OLLAMA_CPU_TARGET}" = "cpu" ]; then
            ...
            #
            init_vars
            CMAKE_DEFS="${COMMON_CPU_DEFS} -DLLAMA_AVX=off -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=off -DLLAMA_FMA=off -DLLAMA_F16C=off ${CMAKE_DEFS}"
            BUILD_DIR="../build/linux/${ARCH}/cpu"
        ...
        if [ "${ARCH}" == "x86_64" ]; then
            ...
            if [ -z "${OLLAMA_CPU_TARGET}" -o "${OLLAMA_CPU_TARGET}" = "cpu_avx" ]; then
                ...
                #
                init_vars
                CMAKE_DEFS="${COMMON_CPU_DEFS} -DLLAMA_AVX=on -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=off -DLLAMA_FMA=off -DLLAMA_F16C=off ${CMAKE_DEFS}"
            ...
            if [ -z "${OLLAMA_CPU_TARGET}" -o "${OLLAMA_CPU_TARGET}" = "cpu_avx2" ]; then
                ...
                #
                init_vars
                CMAKE_DEFS="${COMMON_CPU_DEFS} -DLLAMA_AVX=on -DLLAMA_AVX2=on -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=off -DLLAMA_FMA=on -DLLAMA_F16C=on ${CMAKE_DEFS}"
        ...
        if [ "${ARCH}" == "loongarch64" ]; then
            ...
            if [ -z "${OLLAMA_CPU_TARGET}" -o "${OLLAMA_CPU_TARGET}" = "cpu_lasx" ]; then
                ...
                #
                init_vars
                CMAKE_DEFS="${COMMON_CPU_DEFS} -DLLAMA_AVX=off -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=on -DLLAMA_LSX=off -DLLAMA_FMA=off -DLLAMA_F16C=off ${CMAKE_DEFS}"
            ...
            if 
                [ -z "${OLLAMA_CPU_TARGET}" -o "${OLLAMA_CPU_TARGET}" = "cpu_lsx" ]; then
                #
                # 2021 Loongson LA464
                #
                init_vars
                CMAKE_DEFS="${COMMON_CPU_DEFS} -DLLAMA_AVX=off -DLLAMA_AVX2=off -DLLAMA_AVX512=off -DLLAMA_LASX=off -DLLAMA_LSX=on -DLLAMA_FMA=off -DLLAMA_F16C=off ${CMAKE_DEFS}"
                BUILD_DIR="../build/linux/${ARCH}/cpu_lsx"
                echo "Building LSX LoongArch"
                build
                compress
            fi
        fi
